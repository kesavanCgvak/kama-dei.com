...
class KAASMappingNewEdit{
	//----------------------------------------------------
	constructor(tabID, BotName, BotAlias){
//		this.kaasBase  = new KAASBase(false);
		this.tabID    = tabID;
		this.mapId    = 0;
...
/*
		...
		$("#organizationID").ready(function(){ 
			$("#organizationID").on('change', function(){ myKAAS.getPersona($(this).val(), $("#organizationID option:selected").data('persona')); }); 
			if(orgID!=0){ myKAAS.getPersona(orgID, defaultPersona); }
		});
*/
	}
	//----------------------------------------------------
	...
	getKeys(){
		$.ajax({
			url: apiURL+"/api/dashboard/kaas/setting/getkey",
			type: 'put',
			headers: {
			...
			success: function(retVal){
				kaasClass.kaasBase.AccessKey = retVal.keys[2];
				kaasClass.kaasBase.SecretKey = retVal.keys[4];
				kaasClass.careateIntentsTab();
			},
						...
						var itm ={
							slotName:
									...
									'<button class="btn btn-info btn-mapping" style="float:right;"'+
										'onClick="myKAAS.showSlotTypeValue(\''+stackIntt.tag+'\', \''+stackIntt.data[i].tag+'\')">'+
										'Values'+
									'</button>'+
...
//		$("#krVAL_"+itemID).on('keydown', function(){ $("#"+itemID).data('itemid', 0); });
		$("#krVAL_"+itemID).autocomplete({
			source: function( request, response ) {
				$.ajax( {
					url: apiURL+"/api/dashboard/kaas/mapping/searckkr",
					dataType: "json",
	...
	getData(){
		var tmpThis = this;
		$.ajax({
			url: apiURL+'/api/dashboard/kaas/mapping/getmapped',
			type: 'post',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
...
/*
	getOrganizations() {
		$.get(apiURL + '/api/dashboard/kaas/setting/organization/'+orgID , (res) => {
			if(orgID==0){
			...
			for(var i in res.data ){ 
				...
				$("#ownerId"       ).append(tmp);
				$("#organizationID").append(tmp);
				myKAAS.organizations.push({id:res.data[i].organizationId, caption:res.data[i].organizationShortName});
			}
...
//					$("#searchBox #termOwnersList").append('<option value="'+retVal.data[i].id+'">'+retVal.data[i].text+'</option>'); 
	...
	setJson(mapId, type, name, version, json){
		$.ajax({
			url: apiURL+"/api/dashboard/kaas/mapping/setjson/"+mapId+"/"+type+"/"+name+"/"+version,
			type: 'put',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
	...
	getJson(mapId, type, name, version, inData, calBackFunc){
		...
		$.ajax({
			url: apiURL+"/api/dashboard/kaas/mapping/getjson",
			type: 'get',
			headers: {
//				'Accept': 'application/json',
				'Content-Type': 'application/json'
			...
			success: function(retVal){
				if(retVal.result==0){ 
					if(retVal.data==null){ 
						switch(type){
							case "bot":{ tmpThis.kaasBase.callBot(tmpThis.BotName, tmpThis.BotAlias, inData, calBackFunc); return; }
							case "intent":{ tmpThis.kaasBase.callIntent(name, version, inData, calBackFunc); return; }
							case "slot":{ tmpThis.kaasBase.callSlotType(name, version, inData, calBackFunc); return; }
						}
					}
				...
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						'<div class="modal-header" style="background:#00a6b4;border-radius:5px 0">'+
							'<div>Link Knowledge Records</div>'+
							'<span class="hedarTXT" style="color:yellow;margin-left:20px;"></span>'+
							'<input type="hidden" id="krID" value="0"/>'+
							'<input type="hidden" id="dtID" value="0"/>'+
						'</div>'+
...
if($("#KAAS_MappingNewEdit").length != 0){
	myKAAS = new KAASMappingNewEdit(tabID, BotName, BotAlias);
}
