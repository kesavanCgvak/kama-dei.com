...
class LiveAgentMapping extends DataTable {
	//----------------------------------------------------
	constructor(data){
		super(data);
		...
		$("#ownerId").ready(function(){ 
			...
			$("#ownerId").on('change', function(){
				myClass.getPortals($(this).val(), $("#ownerId option:selected").data('portal'));
			}); 
		});
		$("#portal_id").ready(function(){ 
			$("#portal_id").on('change', function(){
				myClass.getPerson($(this).val());
			}); 
		});
	...
	getOrganizations() {
		$.get(apiURL + '/api/dashboard/portal/getPortalOwner/'+orgID, (res) => {
			...
			if(res.data.length==0){ 
				...
				$("#insertBtn").parent()
					.append("<div style='color:red'>Live Agent Mapping has not been set for this organization, contact kama.ai super-admin to set up Live Agent Mapping</div>");
			}
		});
	}
	getStructure(){
...
//		$("#structure_id"  ).append("<option value=''>Select Structure</option>");
		$.get(apiURL + '/api/dashboard/live_agent/structure' , (res) => {
			if(res.result==1){
				showError(res.msg);
				return;
			}
...
//	showAddDialogHandler(){ window.location.href=this.apiURLBase + '/panel/LiveAgentMapping/mapping/new'; }
	showEditDialogHandler(e){ 
		window.location.href=this.apiURLBase + '/panel/live_agent/mapping/'+$(e.currentTarget).data('itemid'); 
	}
	showDeleteDialogHandler(e){
		...
		showConfirm(function(res){
			if(res){
				$.ajax({
					url: apiURL+'/api/dashboard/live_agent/mapping/delete',
					type: 'delete',
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json'
					...
					success: function(res){
						if(res.result == 0){
							$("#LIVEAGENT_Mapping table").bootstrapTable('refresh');
	...
	setTableWidth(){
		...
		$("#LIVEAGENT_Mapping table").css("min-width", width+"px");
	}
	//----------------------------------------------------
...
//		{ name: 'bot_alias'  , display:'Bot Alias'   , sortable:true, search:true },
	...
	var LiveAgentMappingColumns = new Columns(columns);

	var data = {
		columns: LiveAgentMappingColumns,
		apiURL: apiURL + '/api/dashboard/live_agent/mapping'
	}
	myClass = new LiveAgentMapping(data);
	myClass.createTable('LIVEAGENT_Mapping');
	myClass.setTableWidth();
}
