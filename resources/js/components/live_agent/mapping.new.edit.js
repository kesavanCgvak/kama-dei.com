...
/*
		...
		$("#organizationID").ready(function(){ 
			$("#organizationID").on('change', function(){ myClass.getPersona($(this).val(), $("#organizationID option:selected").data('persona')); }); 
			if(orgID!=0){ myClass.getPersona(orgID, defaultPersona); }
		});
*/
	}
	//----------------------------------------------------
						...
						var itm ={
							slotName:
									...
									'<button class="btn btn-info btn-mapping" style="float:right;"'+
										'onClick="myClass.showSlotTypeValue(\''+stackIntt.tag+'\', \''+stackIntt.data[i].tag+'\')">'+
										'Values'+
									'</button>'+
...
//		$("#krVAL_"+itemID).on('keydown', function(){ $("#"+itemID).data('itemid', 0); });
		$("#krVAL_"+itemID).autocomplete({
			source: function( request, response ) {
				$.ajax( {
					url: apiURL+"/api/dashboard/live_agent/mapping/searckkr",
					dataType: "json",
	...
	getData(){
		var tmpThis = this;
		$.ajax({
			url: apiURL+'/api/dashboard/live_agent/mapping/getmapped',
			type: 'post',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
...
/*
	getOrganizations() {
		$.get(apiURL + '/api/dashboard/live_agent/setting/organization/'+orgID , (res) => {
			if(orgID==0){
			...
			for(var i in res.data ){ 
				...
				$("#ownerId"       ).append(tmp);
				$("#organizationID").append(tmp);
				myClass.organizations.push({id:res.data[i].organizationId, caption:res.data[i].organizationShortName});
			}
...
//					$("#searchBox #termOwnersList").append('<option value="'+retVal.data[i].id+'">'+retVal.data[i].text+'</option>'); 
	...
	setJson(mapId, type, name, version, json){
		$.ajax({
			url: apiURL+"/api/dashboard/live_agent/mapping/setjson/"+mapId+"/"+type+"/"+name+"/"+version,
			type: 'put',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
...
/*
		...
		$.ajax({
			url: apiURL+"/api/dashboard/live_agent/mapping/getjson",
			type: 'get',
			headers: {
//				'Accept': 'application/json',
				'Content-Type': 'application/json'
				...
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						'<div class="modal-header" style="background:#00a6b4;border-radius:5px 0">'+
							...
							'<input type="hidden" id="mkID" value="0"/>'+
						'</div>'+
...
if($("#LIVEAGENT_MappingNewEdit").length != 0){
	myClass = new KAASMappingNewEdit(tabID, BotName, BotAlias);
}
