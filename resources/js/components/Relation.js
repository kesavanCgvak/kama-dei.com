...
//		var icon0 = $('<a></a>').attr({ href: '#', 'data-desc': 'Languages', class: 'link-language', 'data-onlyowner': 1 });
		var icon1 = $('<a></a>').attr({ href: '#', 'data-desc': 'Ext. Data Link', class: 'link-item', 'data-onlyowner': 0 });
		var icon2 = $('<a></a>').attr({ href: '#', 'data-desc': 'KR-KR Link', class: 'link-kr', 'data-onlyowner': 0 });
		var icon3 = $('<a></a>').attr({ href: '#', 'data-desc': 'KR-Term Link', class: 'link-krtoterm', 'data-onlyowner': 0 });
...
*/	
				...
				if(res.result==0){
					for(let i in res.data){
						if(res.data[i].isActive==1 || res.data[i].code=='en'){
							let code = res.data[i].code;
							let caption = res.data[i].name;
							$("#languagesItem #langLanguageCode").append('<option value="'+code+'">'+caption+'</option>');
						}
					}
					$("#languagesItem #langLanguageCode").val('en').change();
					$("#languageCode1").val('en').change();
				}
			});
		}
			...
			$.get(url, function(res){
				...
				if(res.result==0){
					if(res.data!=null){
						...
						validationText = res.data.validationText;
					}
				}
				$("#languagesItem #shortText"     ).val(shortText     ).change();
				$("#languagesItem #optionalText"  ).val(optionalText  ).change();
				$("#languagesItem #validationText").val(validationText).change();
			});
			
			$("#languageCode1").val($('#languagesItem #langLanguageCode').val()).change();
		});
		//--------------------------------------------------------
		...
		$('body').on('change', '#languagesItem #validationText', (e)=>{
			let val = $('#languagesItem #validationText').val().trim();
			$('#languagesItem #btn-validationText').prop('disabled', false);
			if(val.length==0){ $('#languagesItem #btn-validationText').prop('disabled', true); }
			
			$("#validationText1").val($('#languagesItem #validationText').val().trim()).change();
		});
		//--------------------------------------------------------
		...
		$('body').on('keypress', '#languagesItem #validationText', (e)=>{
			let val = $('#languagesItem #validationText').val().trim();
			$('#languagesItem #btn-validationText').prop('disabled', false);
			if(val.length==0){ $('#languagesItem #btn-validationText').prop('disabled', true); }
		...
		$('body').on('click', '#languagesItem #btn-validationText', (e)=>{
			$('#languagesItem #validationText').val('').change();
		});
		//--------------------------------------------------------
/*
...
*/
		...
		$('#languagesItem #langLanguageCode').val('en').prop('disabled', false).change();
		
		$(".col-languageCode1"  ).hide();
		$(".col-shortText1"     ).hide();
		$(".col-optionalText1"  ).hide();
		$(".col-validationText1").hide();
	}
	//----------------------------------------------------
	showAddDialogHandler(e){
		...
		$("#languageCode1").val($('#languagesItem #langLanguageCode').val()).change();		
		$("#optionalText, #shortText, #validationText").val("").change();
					...
					.append(
						...
						.append(
							$("<input>").attr({
								...
								class: 'form-control',
								disabled: true,
								style: "width:100% !important;display:inline-block;margin-right:5px;"
							})
						)
						.append(
							$("<i>").attr({
...
//  { name: 'rightTermName', display: 'Right Term', sortable: true, editable: false },
	...
	{ name: 'validationText1', display: '', sortable: false, editable: true , hidden:true },
