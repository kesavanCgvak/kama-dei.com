...
class RPAMapping extends DataTable {
	//----------------------------------------------------
	constructor(data){
		super(data);
		this.pageSort = 'mappingName';
		this.defaultPersona = [];
		this.addBtnCaption = "Add Mapping RPA";
		...
		$("#ownerId").ready(function(){ 
			...
			$("#ownerId").on('change', function(){
				rpaClass.getPortals($(this).val(), $("#ownerId option:selected").data('portal'));
			}); 
		});
		$("#portal_id").ready(function(){ 
			$("#portal_id").on('change', function(){
				rpaClass.getPerson($(this).val());
			}); 
		});
		$("body").on('change', "#bot_type_id", function(){ 
			$("#structure_id option").remove();
			if($("#bot_type_id").val()!=""){ rpaClass.getStructure(); }
		});
	...
	getOrganizations() {
		$.get(apiURL + '/api/dashboard/portal/getPortalOwner/'+orgID, (res) => {
			...
			if(res.data.length==0){ 
				...
				$("#insertBtn").parent()
					.append("<div style='color:red'>RPA Mapping has not been set for this organization, contact kama.ai super-admin to set up RPA Mapping</div>");
			}
		});
		$("#bot_type_id option").remove();
		$("#structure_id option").remove();
		$.get(apiURL + '/api/dashboard/rpa/types', (res) => {
			if(res.result==1){
				showError(res.msg);
				return;
			}
			for(var i in res.data ){ 
				$("#bot_type_id").append("<option value='"+res.data[i].id+"' >"+res.data[i].name+"</option>");
			}
			$("#bot_type_id").prepend("<option value=''>Select ...</option>");
		});
	}
	getStructure(){
		$("#structure_id option").remove();
		$.get(apiURL + '/api/dashboard/rpa/structure/'+$("#bot_type_id").val() , (res) => {
			if(res.result==1){
				showError(res.msg);
				return;
			}
			...
			for(let i in res.data ){
				{ $("#structure_id").append("<option value='"+res.data[i].id+"'>"+res.data[i].name+"</option>"); }
				structure_id = res.data[i].id;
			}
			$("#structure_id").val(structure_id).change();
		});
	}
...
//		$("#personaName").val('').change();
		...
		$.get(apiURL + '/api/dashboard/portal/portals/'+org_id , (res) => {
			...
			if(defaultPortalId!=0){
				$("#portal_id").val(defaultPortalId).change();
	...
	showAddDialogHandler(){
		...
		if(orgID==0){
			$("#ownerId").val("").change();
			if(defaultOrgId!=0){
				$("#ownerId").val(defaultOrgId).change();
					...
					$.ajax({
						...
						success: function(res){
							if(res.result == 0){
								...
								window.location.href=apiURL + '/panel/LiveAgentMapping/mapping/'+res.id;
							}else{
								showError(res.msg);
...
*/
	//----------------------------------------------------
//	showAddDialogHandler(){ window.location.href=this.apiURLBase + '/panel/LiveAgentMapping/mapping/new'; }
	showEditDialogHandler(e){ 
		window.location.href=this.apiURLBase + '/panel/rpa/mapping/'+$(e.currentTarget).data('itemid');
	}
	showDeleteDialogHandler(e){
		...
		showConfirm(function(res){
			if(res){
				$.ajax({
					url: apiURL+'/api/dashboard/rpa/mapping/delete',
					type: 'delete',
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json'
					...
					success: function(res){
						if(res.result == 0){
							$("#RPA_Mapping table").bootstrapTable('refresh');
	...
	setTableWidth(){
		...
		$("#RPA_Mapping table").css("min-width", width+"px");
	}
	//----------------------------------------------------
...
if($("#RPA_Mapping").length != 0){
	var columns = [
		...
		{ name: 'bot_type_id'  , display:'Bot Type', sortable:false, search:false, editable:true , hidden:true},
		{ name: 'bot_type_name', display:'Bot Type', sortable:true , search:true , editable:false},
...
//		{ name: 'structure'    , display:'Structure', sortable:false, search:false, editable:true },
	...
	var RPAMappingColumns = new Columns(columns);

	var data = {
		columns: RPAMappingColumns,
		apiURL: apiURL + '/api/dashboard/rpa/mapping'
	}
	rpaClass = new RPAMapping(data);
	rpaClass.createTable('RPA_Mapping');
	rpaClass.setTableWidth();
}
