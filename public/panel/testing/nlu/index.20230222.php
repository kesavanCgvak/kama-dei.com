...
<?php /*<input class="form-control" id="portalcode" value="3tEstNL" style="max-width:80%;display:inline-block;" disabled/>*/ ?>
			<select class="form-control" id="portalcode" style="max-width:80%;display:inline-block;" <?=((0==$orgID) ?"disabled" :"");?>>
				<?php
				if($orgID!=0){
					...
					?><option value="">Select Portal</option><?php
				}
				?>
			</select>
		</div>
...
function callSelectOrg(){
	...
	for(let i in portals){
		if(portals[i].orgid==orgid){
			$("#portalcode").append('<option value="'+portals[i].code+'">('+portals[i].code+') '+portals[i].name+'</option>'); 
		}
	}
}
//-----------------------------------------------------
...
$(function(){ 
	...
	$("#orgid").on('change', function(){
		$("#email, #portalcode").prop("disabled", true);
		if($(this).val()!=''){ $("#email, #portalcode").prop("disabled", false); }
	});
	...
	$("#portalcode").on('change', function(){
		$("#btnIdentify").prop("disabled", true);
		if($(this).val()!='' && $("#email").val()!=''){ $("#btnIdentify").prop("disabled", false); }
	});
