...
<form onReset="callRest()">
	<div class="divA">
		...
		<div>
			...
			<select class="form-control" id="portals" style="max-width:80%;display:inline-block;" <?=((0==$orgID) ?"disabled" :"");?>>
				<?php
				if($orgID!=0){
					$portals = \App\Portal::where('organization_id', $orgID)
		...
		<div>
			<label style="display:block;"> Select Portal Type:</label>
			<div class="btn-group" role="group" aria-label="Basic example">
				<button type="button" class="btn btn-info">Original Mode</button>
				<button type="button" class="btn btn-default">Test Mode</button>
			</div>
			<input id="originalMode" />
			<input id="testMode" value="3"/>
			<select class="form-control" id="portalType" style="max-width:80%;display:inline-block;" <?=((0==$orgID) ?"disabled" :"");?>>
				<?php
				$portalTypes = \App\PortalType::orderBy('number', 'asc')->get();
				if(!$portalTypes->isEmpty()){
					foreach($portalTypes as $tmpP){
						?>
						<option value="<?=$tmpP->number;?>">
							<?="({$tmpP->number}) {$tmpP->caption}";?>
						</option>
						<?php
					}
				}
...
function callRest2(flag=0){
	if(flag==0){
		...
		<?php if($orgID==0): ?>
		$("#portalType" ).prop('disabled', true );
		<?php else: ?>
		$("#portalType" ).prop('disabled', false);
		<?php endif; ?>
...
?>
portals.push( {name:'<?=$portal['name'];?>', code:'<?=$portal['portal_number'].$portal['code'];?>', orgid:'<?=$portal['organization_id'];?>' } );
<?php
endforeach;
endif;
?>
...
$(function(){ 
	...
	$("#orgid").on('change', function(){
		$("#email, #portals").prop("disabled", true);
		if($(this).val()!=''){ $("#email, #portals").prop("disabled", false); }
	});
	$("#email").on('change', function(){
		$("#btnIdentify").prop("disabled", true);
		if($(this).val()!='' && $("#portals").val()!=''){ $("#btnIdentify").prop("disabled", false); }
	});
	...
	$("#portalType").on('change', function(){
		let portalCode = $("#portalcode").val().trim().substr(1,5);
		$("#portalcode").val($(this).val()+portalCode);
	});
