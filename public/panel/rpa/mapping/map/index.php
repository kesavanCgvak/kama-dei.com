...
$mapBotRecord = \App\RPAMappingBot::myMap($bot_id);
if($mapBotRecord==null){
	?><script type="application/javascript">window.location.href='<?=env('API_URL');?>/panel/rpa/mapping';</script><?php
	return;
}
...
$orgRPAIntegration = false ;
$orgData = \App\Organization::find($mapBotRecord->ownerId);
if($orgData!=null && $orgData->RPA==1){ $orgRPAIntegration = true ;}

$portalRPAIntegration = false ;
$portalData = \App\Portal::find($mapBotRecord->portal_id);
//if($portalData!=null && $portalData->hasLiveAgent==1){ $portalRPAIntegration = true ;}
if($portalData!=null){ $portalRPAIntegration = true ;}
...
<style>
	...
	.input-group.m-b-1{ width:calc(100% - 175px); max-width: 750px; margin-bottom: 5px; }
...
*/
	...
	tr.handoff{ border-bottom:1px solid #eee; }
	tr.handoff.active{ box-shadow:0 0 10px #aaa; }
	tr.handoff.deactive{ opacity:.4; }
	tr.handoff .btn{ padding:0px; }
	tr.handoff .btn.save,
	tr.handoff .cancel{ display: none; }
	tr.handoff .fa-2x{ font-size: 1.4em; }
...
<div style="width:100%;text-align:left;margin:auto;">
	<?php
	$rpaPortalId = \Session::get('rpaPortalId');
	$href  = env('API_URL')."/panel/rpa/mapping";
	$href2 = env('API_URL')."/panel/rpa/mapping";
	if($rpaPortalId!=null){ $href .="/p/{$rpaPortalId}"; }
//	\Session::put('rpaPortalId', null);
	...
	<span style="float:right">
		...
		<?php if($orgRPAIntegration && $portalRPAIntegration): ?>
	...
	<div class="panel panel-primary">
		<div class="panel-heading" style="font-weight:normal;"><?=$mapBotRecord->bot_type_name;?> Integration</div>
		<div class="panel-body">
			...
			<div class="input-group m-b-1">
				...
				<input class="form-control" id="bot_name" value="<?=$mapBotRecord->bot_name;?>" maxlength="200" disabeled  />
			</div>
			
			<div class="input-group m-b-1">
				<span class="input-group-addon">Bot Alias</span>
				<input class="form-control" id="bot_alias" value="<?=$mapBotRecord->bot_alias;?>" maxlength="200" disabled  />
			</div>
			...
			<div class="input-group m-b-1">
				...
				<p class="form-control" style="width:320px;">
					<?=$mapBotRecord->bot_type_name;?> Integration is <?=(($orgRPAIntegration) ?'Active' :'OFF');?>
				</p>
			</div>
			...
			<div class="input-group m-b-1">
				...
				<input class="form-control" value="<?=$mapBotRecord->portalName;?>" disabled style="width:calc(100% - 321px)"  />
				<p class="form-control" style="width:320px;">
					<?=$mapBotRecord->bot_type_name;?> Integration is <?=(($portalRPAIntegration) ?'Active' :'OFF');?>
				</p>
			</div>
...
<script type="application/javascript">
	...
	var defaultPersona     = null;
	var defaultPersonality = null;
...
<script type="application/javascript">
	...
	$(function(){
		//-----------------------------------------------------
		$("span.content-header-title > span").html("RPA Mapping");
		//-----------------------------------------------------
		$("button.btn.<?=$mapBotRecord->publish_status;?>").show();
		$("#mappingName, #bot_name, #bot_alias").prop('disabled', false);
		if('<?=$mapBotRecord->publish_status;?>'=='Published'){ $("#mappingName, #bot_name, #bot_alias").prop('disabled', true); }
		//-----------------------------------------------------
	...
	function sendToController_mapping(mapping_kr_id, mapping_detail_id, kr_id){
		var data = {};
		data.mapping_kr_id     = mapping_kr_id;
		data.mapping_detail_id = mapping_detail_id;
		data.kr_id             = kr_id;
		data.user_id           = "<?=$session->get('userID');?>";
		if(data.mapping_kr_id!=0 && data.mapping_detail_id==0 && data.kr_id==0){
	...
	function sendToController_mapping_(data){
		$.ajax({
			url: apiURL+'/api/dashboard/rpa/mapping/mapped',
			type: 'put',
			headers: {
...
//					if(data.mappingBot_id==0 || data.mapping_kr_id==0){ window.location.reload(); }
					if(data.mapping_detail_id==0 || data.mapping_kr_id==0){
						if(data.mapping_detail_id==0 && data.mapping_kr_id!=0){ window.location.href="<?="{$href2}/{$bot_id}";?>#-1"; }
						else{
							if(res.added==1)
	...
	function setPublishStatus(status, redo=0){
		var data = {};
		data.mappingName    = $("#mappingName").val().trim();
		data.bot_name       = $("#bot_name"   ).val().trim();
		data.bot_alias      = $("#bot_alias"  ).val().trim();
		...
		if(data.bot_name==""){
			myClass.showError("invalid bot name");
			$("#bot_name").focus();
			return;
		}
		if(data.bot_alias==""){
			myClass.showError("invalid bot alias");
			$("#bot_alias").focus();
			return;
		}
		...
		data.redo = redo;
		$.ajax({
			url: apiURL+'/api/dashboard/rpa/mapping/'+status,
			type: 'put',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
			...
			success: function(res){
				if(res.result == 0){
					...
					$("#mappingName, #bot_name, #bot_alias").prop('disabled', false);
					if(status=='Published'){ $("#mappingName, #bot_name, #bot_alias").prop('disabled', true); }
					$("#publish_status").html("Status: "+status);
				}else{
					if(res.result==1){
						myClass.showError(res.msg);
					}else{
						myClass.showConfirm(function(ret){
							if(ret){ setPublishStatus(status, 1); }
						}, res.msg, "Unpublish the others and publish this", "Close", "btn-danger btnUOPT");
						$(".btnUOPT").parent().parent().css("padding", "15px 10px");
						$(".btnUOPT").parent().css("display", "flow-root");
						$(".btnUOPT").css("margin", "5px 0").css("padding", "10px 0");

						$(".btnUOPT").parent().find("button").css("width", "100%");
						let btnNO = $(".btnUOPT").parent().find(".btn-no");
						$(btnNO).insertAfter($(".btnUOPT"));
					}
				}
			},
			error: function(e){ myClass.showError(e.status+" : "+e.statusText); }
	...
	function callAddNew(parent_id, other=""){
		...
		$.ajax({
			url: apiURL+'/api/dashboard/rpa/mapping/newrow',
			type: 'put',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
			...
			success: function(res){
				if(res.result == 0){
					let url = "/panel/rpa/mapping/<?=$bot_id?>";
//					if(parent_id==0){ url+="."+res.id; }
//					else{ url+="."+parent_id+"."+res.id; }
					url+="."+res.parent;
					window.location.href= url;
//					window.location.reload();
//					window.location.href = "/panel/rpa/mapping/<?=$bot_id?>"+((parent_id==0) ?"" :"."+parent_id);
				}else{ myClass.showError(res.msg); }
			},
			error: function(e){ myClass.showError(e.status+" : "+e.statusText); }
		});
...
<?php include_once("layer_1.1.php"); ?>
