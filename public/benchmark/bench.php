...
print "<pre>".$lf;
printf('%s%s', str_pad(sprintf("MySQL Config"), $w, '-', STR_PAD_BOTH), $lf);
foreach($args as $key=>$arg){
	if($key!='mysql_password' && $key!='multiplier'){
		printLine($key, $arg);
	}
}
...
if (!empty($additionalBenchmarks)) {
    ...
    foreach ($additionalBenchmarks as $name => $benchmark) {
        $currentBenchmark = $name;
		if($name=='mysql::server_info'){
			printLine($name, $benchmark($multiplier));
		}
		else{
			$time = runBenchmark($stopwatch, $benchmark, $multiplier);
			printLine($name, $time);
