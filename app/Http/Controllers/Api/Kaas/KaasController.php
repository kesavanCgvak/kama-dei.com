 ...
 *     1. read inquiry
 *     2. if solution button is pressed
 *           process solution button value
 *           if mapping found, exit
 *     3. if linking term button is pressed
 *           process linking term button value
 *           if mapping found, exit
 *     4. if inquiry and ElicitSlot
 *           process ElicitSlot
 *           if mapping found, exit
 *     5. if inquiry
 *           find problems
 *           search mapping with problem krId
 *           if mapping found, exit 
 *           search solution
 *     6. send response           
 *
...
namespace App\Http\Controllers\Api\Kaas;

use Illuminate\Http\Request;
use App\Chatbox;
use App\Controllers;
use App\KaaS\KaaSClass;
...
class KaasController extends \App\Http\Controllers\Controller{
   ...
   //--------------------------------------------------------------------------------------
   //   READ PARAMETERS FROM NLU LAYER
   //--------------------------------------------------------------------------------------
	public function newShow(Request $request){

      // regular chatbot parameters
      $userId  = trim($request->input('userid' ));
      $orgId   = trim($request->input('orgid'  ));
      $state   = trim($request->input('state'  )); 
      $inquiry = trim($request->input('inquiry'));

      // Kaas specific parameters
      $botName   = trim($request->input('botName'  ));
      $botVersion= trim($request->input('botVersion' ));
      $botAlias  = trim($request->input('botAlias' ));
      $botState  = trim($request->input('botState')); 
      $intentName = trim($request->input('intentName' ));
      $slotName  = trim($request->input('slotName'  ));


      if ($botName == NULL or empty($botName)) {
          $botName = "";
      }
      if ($botVersion == NULL or empty($botVersion)) {
          $botVersion = "";
      }
      if ($botAlias == NULL or empty($botAlias)) {
          $botAlias = "";
      }
      if ($intentName == NULL or empty($intentName)) {
          $intentName = "";
      }
      if ($slotName == NULL or empty($slotName)) {
          $slotName = "";
      }
      ...
      $utterance_orig = "";                     // default utterance_orig
      $utterance_3PB  = "";                     // default utterance_3PB
      $termSet   = "";                          // default termSet


      foreach($inquiry2 as $rs0){ 
         $message2 = $rs0['message'];
         $message99 = $rs0['message'];
         if (isset($rs0['utterance'])) {           // read utterance if it is set
             $utterance = $rs0['utterance'];
         }


         if (isset($rs0['utterance_orig'])) {       // read utterance orig if it is set
             $utterance_orig = $rs0['utterance_orig'];
         }

         if (isset($rs0['utterance_3PB'])) {       // read utterance 3PB if it is set
             $utterance_3PB = $rs0['utterance_3PB'];
         }
      ...
      if ($state == 300) {  
         $messageType = gettype($message2);
         if ($messageType == "string") {                     // message is string
            $substr =    substr($message2,0,5); 
      ...
      // state = 300 ->read slide bars parameters
      // message and answers (term values and scalars)
      ...
      if ($state == 300) { 
        ...
        $inquiry0['message'] = $message2;  
        $inquiry0['slidebar'] = $stringBar; 


      } else {
      // normal input: only message
        $inquiry0 = $message2;    	
      }
    ...
    $returnShow = $this->show($userId, $orgId,$state, $inquiry0, $aLang, $api_key, $utterance, $termSet, 
       $utterance_orig, $utterance_3PB, $botName, $botAlias, $botState, $intentName,$slotName);
  ...
  //--------------------------------------------------------------------------------------
  //   MAIN LOGIC OF THE KAAS CONTROLLER
  //--------------------------------------------------------------------------------------
	...
	///////////////
        ...
        $oKaasClass               = new KaaSClass($botName, $botAlias, $orgid); 

        $useKaas  = 1;              // 0: use kama-DEI data;  1: use mapping data
        $jkaasTMP = "";             // json solution for mapping data
        $aKaasTMP = "";             // array solution for mapping data
        $solutionText = "";         // solution from Kama-DEI data
        $valueName = "";
        //$valueName = $oFunctionHelper->getRightmostTerm($inquiry);
        //$valueNameId  = $oTerm->retrieveTermIdByName($valueName);
        ...
        // get inquiry parameters
        $slidebar ="";
        $inState = $state;         // save incoming state  (state from previous request) 
        ...
        $cmdLargestIEError = "cmd_largestIE_ERROR";
        ...
        $lastState = 0;
        $kaasFilterId = 122;          
        $relationLinkId = 0;          // mapping link from solution_relation
        $optionLinkId = 0;            // mapping link from solution_option
        $kaasLinkType = 0;            // 1: solution button; 2: linking term button

        // --- Advanced search using termSet  ------------------------
        $enableAdvancedSearch  = 1;      //  0: diseabled; 1:enabled
        ...
        *---------------------------------------------------------------*/
        ...
        // Prepara link KamaDEI - Kaas    
        $kaasLinkName ="";          // valueName from button value 
        $kaasLinkId   = 0; 

        if ($useKaas == 1) {        // use kaas mapping

            if ($flowHint) {
               $kaasLinkType = 0;
               $kaasLinkType = $oFunctionHelper->getKaasLinkType($inText);

               if ($kaasLinkType == 1) {             // get mapping link in solution_relation
                  $relationLinkId = $oSolutionRelation->getKaasLink($userid,$inText);
               }

               if ($kaasLinkType == 2) {             // get mapping link in solution_option
                  $optionLinkId = $oSolutionOption->getKaasLink($userid,$inText);
               }
            }
        }
        ...
        if ($useKaas == 1) {    // use kaas mapping
            if ($kaasLinkType > 0) {
                $state = 11;          // priority 1 and 2: process clicked button 
            } else {
                if ($botState == "ElicitSlot") {
                   $state = 13;       // priority 3: process ElicitSlot 
                }
            }
        }
        //////////////////////////////////////////////////
              ...
              */        
              ...
              case 4:
               ...
               // $sSegment  = $oFunctionHelper->extractSegmento($inputText); 
                ...
                $sTerm    = $oFunctionHelper->getRightmostTerm($inquiry);
              ...
              ////////////////////////////////////////////////////////////
              /**  State  11                                           **/  
              /*  Link from kamaDEI solution to Kaas                    */        
              /*  change verb. use relation type synonym:               */        
              case 11: 

                if ($kaasLinkType == 1) {    // solution button was clicked
                    $krId = $relationLinkId;
                }

                if ($kaasLinkType == 2) {    // linking term button was clicked
                    $krId = $optionLinkId;
                }

                $oKaasClass->findIntent($krId);    // find intent krid
                $jkaasTMP0  = $oKaasClass->getData();   
                $jkaasTMP   = json_encode($jkaasTMP0, JSON_UNESCAPED_SLASHES );
                $aKaasTMP   = $jkaasTMP;
                $aKaasTMP   = json_decode( utf8_encode($aKaasTMP), true );

                if (isset($aKaasTMP['intent'])) {  
                    $intentName = $aKaasTMP['intent'];
                    $state = 804;
                    $inLoop = 0;             // exit loop
                }  else {
                    $state = $flowState;
              ...
              ////////////////////////////////////////////////////////////
              /**  State  13                                           **/  
              /*   Process inquiry with ElicitSlot                      */        
              /*                                                        */        
              case 13: 

                // variables used for Kaas ElicitSlot;
                $slotState = 0;
                $valueName = "";   // value name used for searching mapping data
                $aTermSet = explode(",",$termSet); 

                // process termSet array
                $aLen = count($aTermSet);     // termSet array length
                $aTermSet = array_reverse($aTermSet);  // process from right to left
              
                for ($i=0; $i< $aLen; $i++) {
                    $valueName = $aTermSet[$i];

                    $oKaasClass->findSlotName($intentName,$slotName,$valueName);
                    ...
                    // if Value_KrId was found  
                    if (isset($aKaasTMP['slots'][0]['Value_krId'])  and $aKaasTMP['slots'][0]['Value_krId'] > 0) {    
                        $slotState = 805;
                        $i = $aLen;            // exit for loop
                    }
                }

                // if no intent,slot,value mapping is found, process synonyms
                if ($slotState == 0) {
                    $aTermSynonym = $oInferenceHelper->getSynonymArray($aTermSet);

                    // process termSet synonym array
                    $aLen = count($aTermSynonym);     // aTermSynonym array length
                    for ($i=0; $i< $aLen; $i++) {
                        $valueName = $aTermSynonym[$i];
                ...
                if ($slotState == 805) {
                    $state = 805;             // ELicitSlot solution found
                    $inLoop = 0;              // Exit and send mapping response 
                } else {
                    $state = $flowState;     // continue regular kama-DEI processing                
                }
              ...
              /////////////////   kaas mapping processing   /////////////////////////////
                $mappingState = 0;
                $problemRS = $oSolutionFact->getFactKR($userid);
                foreach($problemRS as $rs0) {
                  $krId = $rs0->sfRelationId;
                  if ($mappingState == 0) {
                    $oKaasClass->findIntent($krId);    // find intent krid
                    $jkaasTMP0  = $oKaasClass->getData();   
                    $jkaasTMP   = json_encode($jkaasTMP0, JSON_UNESCAPED_SLASHES );
...
/////////////////END OF SEGMENT TESTING ////////////////////////////////////////77
              ...
              case 998:
                ...
                if ($advancedSearchOn == 0)  {
                   if ($enableAdvancedSearch == 1) {      // advancedSearch is enabled
                      $inquiry = $termSet;
                      $sSegment = $termSet;
                      $advancedSearchOn = 1;
                      $state = 4;  
    ...
    /////////////// START OF SEND RESPONSE /////////////
        ...
        switch ($state) { 
                   ...
                   [
                      'response' => 
                         [
                          ...
                          'slotName'     => $slotName,
                          'utterance_orig' => $utterance_orig,
                          'utterance_3PB'  => $utterance_3PB,
                          'language'     => $jsonLang,
                          'contentType'  => $contentType,                          
            ...
            ///////////////////////////////////////////////////////////////////   
            /**  State 804                                                  **/           
            /*   exit with solution from KaasClass                           */ 
            /*   ElicitIntent. intent from mapping data                      */          
            case 804:
            ...
            /**  State 805                                                  **/           
            /*   exit with solution from KaasClass                           */   
            /*   ElicitSlot. intent and slot from mapping data               */          
            case 805:
