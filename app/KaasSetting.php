...
class KaasSetting extends Model {
	//--------------------------------------------------------------------
	...
	protected $fillable = ['id', 'org_id', 'aws_customer_id', 'personalityId', 'kaasPersonalityID', 'kaasUserID', 'user_id', 'last'];
	//--------------------------------------------------------------------
...
/*
				->leftJoin('kamadeiep.api_key_manager as apikeys', function($leftJoin){
					$leftJoin
						->on('setting.kaasUserID', '=', 'apikeys.userID')
						->on('apikeys.portal_code', '=', \DB::raw("'2KAAS!'"));
				})
*/
				->where(
...
/*
							...
							->orwhere('kaasPRS.personalityName'   , 'like', "%{$value}%")
