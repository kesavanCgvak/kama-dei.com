 ...
 *  File          : Term.php        
 *	Type          : Model
 *  Function      : Provide  functions for manipulating term table.
...
class Term extends Model
{
	...
	protected $modifiers  = ['termId', 'termName', 'termIsReserved', 'ownership',
                           	'ownerId', 'dateCreated', 'lastUserId'];
    ...
    //--------------------------------------------------------------------
    public function getLikeName($orgid,$termName,$rtgId)
    ...
    //--------------------------------------------------------------------
    public function retrieveMultiTermName($orgId,$termId,$targetLang,$baseLang="en")
     {
     ...
     /*
        // term translation is disabled 
        // this query finds termId in two steps: join term_translation->term, and term    

        $rs =  $this->leftJoin('term_translation', function($join) use ($termId,$targetLang)
              {
                $join->on('term.termId', '=', 'term_translation.termId');
              })
              ->where('term.termId', '=', $termId)
              ->where('term_translation.termLanguage', '=', $targetLang)
            ->get();  
...
*/  
        ...
        $aName['termName']= $termName;
        $aName['language']= $language;     
    ...
    //--------------------------------------------------------------------
    public function getLikeRightName($termName)
     {
        return $this->where('termName', '=',$termName)
                   ->orWhere('termName', 'LIKE',"%$termName")
                    ->get(); 
     }

    //--------------------------------------------------------------------
    public function TermTranslation(){
        return $this->hasMany('App\Models\TermTranslation', 'termTranslationId');
    }
